...
if not opt then
  ...
  cmd:option('-domainLambda', 5, 'regularization term for transfer learning')
...
nTrainTarget = 6000
nValidTarget = 6000
EEGDataset2 = torch.load('Eyes-closed/Session2/EEG_Session2.torch')
...
print(EEGDataset1[1][1])
print(EEGDataset2[1][1])
-- print(sourceTrainSet:narrow(1,1,10))
-- print(targetTrainSet:narrow(1,1,10))
...
for i = 1,opt.maxEpoch do
  ...
  if i == opt.maxEpoch then
    local DomainlossValid = sourceDomCostValid+targetDomCostValid
    local DomainlossTrain = sourceDomCost+targetDomCost
    file = csvigo.load("exp_23_09_16.csv")
    ...
    table.insert(file["DomainlossValid"], DomainlossValid)
    table.insert(file["DomainlossTrain"], DomainlossTrain)
    ...
    csvigo.save("exp_23_09_16.csv", file)
  end
