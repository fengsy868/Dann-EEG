...
gen = torch.Generator(gen,opt.seed)
...
sourceTrainSet = torch.CudaTensor(nTrainS1, nInputEEG)
sourceTrainSet:copy(EEGDataset1[1]:narrow(1,1,nTrainS1):float())
sourceTrainSetLabel = torch.CudaTensor(nTrainS1, nInputEEG)
sourceTrainSetLabel:copy(EEGDataset1[1]:narrow(1,1,nTrainS1):float())

sourceValidSet = torch.CudaTensor(nTrainS1,nInputEEG)
sourceValidSet:copy(EEGDataset1[1]:narrow(1,nTrainS1+1,nValidS1):float())
sourceValidSetLabel = torch.CudaTensor(nValidS1, nInputEEG)
sourceValidSetLabel:copy(EEGDataset1[1]:narrow(1,nTrainS1+1,nValidS1):float())

sourceInputs = torch.CudaTensor(opt.batchSize,sourceTrainSet:size(2)) 
sourceLabels = torch.CudaTensor(opt.batchSize,sourceTrainSet:size(2))                                        
...
targetTrainSet = torch.CudaTensor(nTrainS2, nInputEEG)
targetTrainSet:copy(EEGDataset2[1]:narrow(1,1,nTrainS2):float())
targetTrainSetLabel = torch.CudaTensor(nTrainS2, nInputEEG)
targetTrainSetLabel:copy(EEGDataset2[1]:narrow(1,1,nTrainS2):float())

targetValidSet = torch.CudaTensor(nTrainS2,nInputEEG)
targetValidSet:copy(EEGDataset2[1]:narrow(1,nTrainS2+1,nValidS2):float())
targetValidSetLabel = torch.CudaTensor(nValidS2, nInputEEG)
targetValidSetLabel:copy(EEGDataset2[1]:narrow(1,nTrainS2+1,nValidS2):float())

targetInputs = torch.CudaTensor(opt.batchSize,targetTrainSet:size(2)) 
targetLabels = torch.CudaTensor(opt.batchSize,targetTrainSet:size(2)) 
...
function train()
   ...
   for t = 1,sourceTrainSet:size(1),opt.batchSize do
     ...
     local feval = function(x)
       ...
       local domCost = domainClassifierCriterion:forward(domPreds,torch.CudaTensor(domPreds:size(1),1):fill(1)) -- TODO: ugly, replace with two unique allocations
       local domDfdo = domainClassifierCriterion:backward(domPreds,torch.CudaTensor(domPreds:size(1),1):fill(1))
       ...
       local targetDomCost = domainClassifierCriterion:forward(targetDomPreds,torch.CudaTensor(targetDomPreds:size(1),1):fill(0)) -- TODO: ugly, replace with two unique allocations
       local targetDomDfdo = domainClassifierCriterion:backward(targetDomPreds,torch.CudaTensor(targetDomPreds:size(1),1):fill(0))
...
for i = 1,opt.maxEpoch do
   ...
   local sourceDomCost = domainClassifierCriterion:forward(sourceDomPreds,torch.CudaTensor(sourceDomPreds:size(1),1):fill(1)) -- TODO: ugly, replace with two unique allocations

   local targetFeats = featExtractor:forward(targetTrainSet)
   local targetDomPreds = domainClassifier:forward(targetFeats)
   local targetDomCost = domainClassifierCriterion:forward(targetDomPreds,torch.CudaTensor(targetDomPreds:size(1),1):fill(0)) -- TODO: ugly, replace with two unique allocations

   -- trainConfusion:batchAdd(sourceDomPreds, torch.CudaTensor(sourceDomPreds:size(1),1):fill(1))
   print("EPOCH: " .. i)
   ...
   local sourceDomPreds = domainClassifier:forward(sourceFeats)
   local sourceDomCostValid = domainClassifierCriterion:forward(sourceDomPreds,torch.CudaTensor(sourceDomPreds:size(1),1):fill(1)) -- TODO: ugly, replace with two unique allocations

   local targetFeats = featExtractor:forward(targetValidSet)
   local targetDomPreds = domainClassifier:forward(targetFeats)
   local targetDomCostValid = domainClassifierCriterion:forward(targetDomPreds,torch.CudaTensor(targetDomPreds:size(1),1):fill(0)) -- TODO: ugly, replace with two unique allocations

   -- validConfusion:batchAdd(targetDomPreds, torch.CudaTensor(targetDomPreds:size(1),1):fill(0))
   -- print(validConfusion)
   print(" + Valid MSE loss " .. validLoss)

   print(" + Valid domain loss " .. sourceDomCostValid+targetDomCostValid)
