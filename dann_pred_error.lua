...
if not opt then
  ...
  cmd:option('-saveModel', true, 'flag for saving the model on disk at each epoch, if improvement')
  cmd:option('-save', 'results', 'subdirectory to save/log experiments in')
  cmd:option('-hiddenLayerUnits', 50 , 'Number of hidden layer units')
...
trainSet = torch.cat(sourceTrainSet, targetTrainSet, 1)
validSet = torch.cat(sourceValidSet, targetValidSet, 1)
...
gradParams = torch.Tensor(featExtractorGradParams:size(1)+labelPredictorGradParams:size(1)+domainClassifierGradParams:size(1))
--
...
function train()
  ...
  for t = 1,sourceTrainSet:size(1),opt.batchSize do
    ...
    local feval = function(x)
      ...
      local domCost = domainClassifierCriterion:forward(domPreds,torch.Tensor(domPreds:size(1)):fill(1)) -- TODO: ugly, replace with two unique allocations
      ...
      local targetDomCost = domainClassifierCriterion:forward(targetDomPreds,torch.Tensor(targetDomPreds:size(1)):fill(0)) -- TODO: ugly, replace with two unique allocations
      local targetDomDfdo = domainClassifierCriterion:backward(targetDomPreds,torch.Tensor(targetDomPreds:size(1)):fill(0))
...
for i = 1,opt.maxEpoch do
  ...
  local sourceDomCost = domainClassifierCriterion:forward(sourceDomPreds,torch.Tensor(sourceDomPreds:size(1)):fill(1)) -- TODO: ugly, replace with two unique allocations
  ...
  local targetDomPreds = domainClassifier:forward(targetFeats)
  local targetDomCostValid = domainClassifierCriterion:forward(targetDomPreds,torch.Tensor(targetDomPreds:size(1)):fill(0)) -- TODO: ugly, replace with two unique allocations
...
-- end
  ...
  --   file = csvigo.load("exp_22_07_16.csv")
  --   table.insert(file["TrainMSE"], validLoss)
  ...
  --   csvigo.save("exp_22_07_16.csv", file)
  -- end
...
print('Training finished, Time Total Used:'.. sys.clock()-tick2 .. 's')


function mse( preds, targets )
  count = 0
  for i=1,preds:size(1) do
    count = count + math.sqrt((preds[i]-targets[i])^2)
  end
  return count/preds:size(1)
end

trainFeats = featExtractor:forward(trainSet)
trainPred = labelPredictor:forward(trainFeats)

validFeats = featExtractor:forward(validSet)
validPred = labelPredictor:forward(validFeats)
file = csvigo.load("exp_22_07_16_2.csv")
...
csvigo.save("exp_22_07_16_2.csv", file)
